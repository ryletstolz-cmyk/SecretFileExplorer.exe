...
📄 README — Secret File Explorer
🔐 Secret File Explorer
A small app that lets you browse your computer with a custom file explorer, pick a file or folder, and then open it in Windows Explorer with special “highlight” mode.

✨ Features
Custom file explorer (not the normal Windows one)
...
📁 Files
SecretFileExplorer.ps1 → Main PowerShell script

SecretFileExplorer.exe → EXE version (made from the script)

▶️ How to Run
If using the EXE:
Double‑click SecretFileExplorer.exe
...
⚠️ Notes
Some antivirus programs block homemade EXE files.
...
Browse → Opens the custom file explorer
